...
export async function getCart(): Promise<CarritoDeCompra | null> {
  ...
  let cart: CarritoConProductos | null = null;
...
export async function mergeAnonymousCartIntoUserCart(userId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    if (userCart) {
      ...
      //delete all the items from the user cart and put the merged items
...
//delete the local cart that we fetched from the cookie
